...
void vApplicationStackOverflowHook(TaskHandle_t xTask,
     ...
     * more. */
    memset((void *)crash_info_ram_p, 0,
           sizeof crash_info_ram);  // GCC recognized as built-in function
    snprintf((char *)crash_info_ram.task_name, sizeof crash_info_ram.task_name, "%s",
             pcTaskName);
    crash_info_ram.magic = crash_magic_stack_overflow;
...
void vApplicationMallocFailedHook(void) {
    ...
    snprintf((char *)crash_info_ram.task_name, sizeof crash_info_ram.task_name, "%s",
             pcTaskGetName(NULL));
    crash_info_ram.magic = crash_magic_malloc_failed;
    crash_info_ram.timestamp = epochtime;
    crash_info_ram.xor_checksum =
        crc7((uint8_t *)&crash_info_ram, offsetof(crash_info_t, xor_checksum));
    __DSB();  // make sure all data is really written into the memory before
...
void error_handler(void) {
    ...
    snprintf((char *)crash_info_ram.task_name, sizeof crash_info_ram.task_name, "%s",
             pcTaskGetName(NULL));
    crash_info_ram.magic = crash_magic_err_handler;
    crash_info_ram.timestamp = epochtime;
    crash_info_ram.xor_checksum =
        crc7((uint8_t *)&crash_info_ram, offsetof(crash_info_t, xor_checksum));
    __DSB();  // make sure all data is really written into the memory before
        ...
        case crash_info_magic:
            ...
            switch (crash_info_p->magic) {
                ...
                case crash_magic_stack_overflow:
                    nwrit += snprintf(buf + nwrit, buf_sz - nwrit, "\tStack Overflow.");
                    next = crash_info_task_name;
                    break;
                case crash_magic_malloc_failed:
                    nwrit += snprintf(buf + nwrit, buf_sz - nwrit, "\tMalloc failed.");
                    next = crash_info_task_name;
                    break;
